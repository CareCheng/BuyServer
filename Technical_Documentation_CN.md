...
│  ┌────────────────────────┴─────────────────────────┐        │
│  │                 Cache 层（缓存抽象）               │        │
│  │    CacheManager → Redis / LocalCache (fallback)  │        │
│  └────────────────────────┬─────────────────────────┘        │
│                           │                                  │
│  ┌────────────────────────┴─────────────────────────┐        │
│  │                Repository 层                      │        │
...
KamiServer 实现了一套完整的缓存抽象层，支持 Redis 和本地内存缓存两种模式，并具备自动故障转移能力。
...
│  ┌─────────────────────────────────────────────────────────┐ │
│  │                  Cache Interface                         │ │
│  │  Get | Set | Delete | Exists | Flush | Keys | Info      │ │
│  └────────────────────────┬────────────────────────────────┘ │
│                           │                                  │
│       ┌───────────────────┴───────────────────┐              │
│       ▼                                       ▼              │
│  ┌─────────────┐                       ┌─────────────┐       │
│  │ RedisCache  │ ◄─── failover ────► │ LocalCache   │       │
│  │ (Primary)   │                       │ (Fallback)  │       │
│  └─────────────┘                       └─────────────┘       │
│       │                                       │              │
│       ▼                                       ▼              │
...
| 模式 | 配置值 | 说明 |
|------|--------|------|
| 单机模式 | `standalone` | 连接单个 Redis 实例，适合开发和小型部署 |
| 哨兵模式 | `sentinel` | 支持 Redis Sentinel，自动主从切换 |
| 集群模式 | `cluster` | 支持 Redis Cluster，数据分片 |
...
redis:
  ...
  mode: standalone               # 运行模式：standalone/sentinel/cluster
  address: localhost:6379        # Redis 地址
  password: ""                   # 认证密码
  database: 0                    # 数据库编号（单机/哨兵模式）
  key_prefix: "kami:"            # 键前缀
  pool_size: 10                  # 连接池大小
  ...
  # 哨兵模式专用
  master_name: mymaster          # Master 名称
  sentinel_addrs:                # 哨兵节点地址列表
    - 192.168.1.1:26379
    - 192.168.1.2:26379
```
...
当 Redis 连接出现问题时，系统会自动降级到本地内存缓存：

1. **检测机制**：每次 Redis 操作失败时记录错误
2. **降级条件**：连续失败超过阈值或连接断开
3. **降级行为**：自动切换到 LocalCache，所有操作继续正常工作
4. **恢复检测**：后台定时检测 Redis 可用性
5. **自动恢复**：Redis 恢复后自动切换回来

```go
// CacheMetrics 记录缓存运行指标
type CacheMetrics struct {
    Hits           int64         // 命中次数
    Misses         int64         // 未命中次数
    Failovers      int64         // 故障转移次数
...
系统通过仪表盘 API 提供丰富的缓存统计信息：

| 指标 | 说明 |
|------|------|
| `mode` | 当前缓存模式：redis-standalone/redis-sentinel/redis-cluster/local |
| `status` | 连接状态：connected/degraded/disconnected |
| `version` | Redis 版本（仅 Redis 模式） |
| `uptime` | 运行时间 |
| `hit_rate` | 缓存命中率 |
| `hits` | 命中次数 |
| `misses` | 未命中次数 |
...
| `rdb_enabled` | RDB 持久化状态 |
| `aof_enabled` | AOF 持久化状态 |
...
```json
{
  ...
  "dashboard": {
    ...
    "misses": 571,
    "ops_per_second": 45.2,
    "memory_used": "128.5 MB",
    "memory_peak": "256.0 MB",
    "keys_count": 1250,
    "connected_clients": 8,
    "failovers": 0
  }
...
```
GET /api/admin/redis/keys?pattern=user:*&limit=100
```
...
```
GET /api/admin/redis/key/info?key=user:123:profile
```
...
```
DELETE /api/admin/redis/key?key=user:123:profile
```

#### 24.6.5 刷新缓存统计

```
POST /api/admin/redis/refresh
```

#### 24.6.6 清空所有缓存

```
POST /api/admin/redis/flush?confirm=true
```

⚠️ **警告**：此操作会清空所有缓存数据，用户会话不受影响（存储在数据库中）。

### 24.7 本地缓存说明

当 Redis 未启用或不可用时，系统使用本地内存缓存：
...
// 设置缓存（带过期时间）
err := cm.Set(ctx, "user:123:profile", userData, 1*time.Hour)

// 获取缓存
var user User
found, err := cm.Get(ctx, "user:123:profile", &user)
if err != nil {
    // 处理错误
}
if !found {
...
// 删除缓存
err := cm.Delete(ctx, "user:123:profile")

// 检查键是否存在
exists, err := cm.Exists(ctx, "user:123:profile")

// 获取匹配的键列表
keys, err := cm.Keys(ctx, "user:*")
...
1. **键命名规范**：使用冒号分隔的层级结构，如 `module:entity:id:field`
2. **过期时间设置**：根据数据更新频率设置合理的 TTL
3. **缓存穿透防护**：对于不存在的数据也进行短时间缓存
4. **缓存更新策略**：数据变更时主动删除相关缓存
5. **监控告警**：关注命中率下降和内存使用异常
